...
import AlbumCard from './AlbumCard'; // Import component đã sửa
...
const TrendingAlbums: React.FC<TrendingAlbumsProps> = ({ data }) => {
  const scrollRef = useRef<HTMLDivElement>(null);

  const scroll = (direction: 'left' | 'right') => {
    if (scrollRef.current) {
      const { current } = scrollRef;
      const scrollAmount = 199; // Giữ nguyên thông số của bạn
      if (direction === 'left') {
  ...
  return (
    <section className="w-full max-w-362.5 mx-auto mt-12">
      ...
      <div className="flex items-center justify-center gap-6 mt-10 w-full">
        ...
        >
          {/* Map qua data thật được truyền vào */}
          {data.map((album) => (
